...
def get_all_positive_reviews():
    reviews = train_collection.find({"row.label": 1})
    return format_reviews(reviews)


def get_all_negative_reviews():
    reviews = train_collection.find({"row.label": 0})
    return format_reviews(reviews)


def get_all_reviews():
    reviews = train_collection.find()
    return format_reviews(reviews)
...
def get_pos_or_neg_reviews(words_list: list, binary_pos_or_neg: int) -> list:
    ...
    reviews = train_collection.find(query)  # ~ O(N) ~
    return format_reviews(reviews)
